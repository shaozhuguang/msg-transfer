...
public class ReceiverHandler extends ChannelInboundHandlerAdapter implements Closeable {
    ...
    public void addListener(String listenerKey, ReplyListener replyListener) {
        allReplyListeners.put(listenerKey, replyListener);
    }

    public void removeListener(String listenerKey) {
        this.allReplyListeners.remove(listenerKey);
    }

    @Override
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {

        LOGGER.debug("{} Receive Biz Message -> {}", this.localNode.toString(), msg.toString());
        // 有数据接入
        ...
        if (tm == null) {
            ...
            if (messageType.equals(TransferMessage.MESSAGE_TYPE.TYPE_REQUEST)) {
                ...
                // 其他消息只需要打印日志即可
                LOGGER.error("Receive Message Type = {}", messageType);
            }
        }
    }
    ...
    private void executeResponse(final TransferMessage transferMessage) {
        msgExecutorPool.execute(() -> {
            ...
            if (replyListener != null) {
                // 判断Class<?>
                if (replyListener instanceof CallBackReplyListener) {

                    CallBackReplyListener callBackReplyListener = (CallBackReplyListener) replyListener;

                    // 填充对应的结果
                    callBackReplyListener.reply(transferMessage.load());

                    CallBackReplyListener.MANAGE_TYPE manageType = callBackReplyListener.manageType();
                    ...
                    if (manageType != null) {
                        switch (manageType) {
                            ...
                            default:
                                // 打印日志
                                LOGGER.debug("Reply Listener Manage Type = {}", manageType);
                                break;
                        }
                    }
                } else {
                    replyListener.reply(transferMessage.load());
                    // 不需要移除，移除操作由外部进行
                }
            }
        });
